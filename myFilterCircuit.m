...
function Vout = myFilterCircuit(Vin,h)
    %Set Vars 
    R = 1;    % Resistance (Ohms)
    freq = 600;  % Note To Play (hz)
    bandwidth = 200; % Range (hz)

    %Calculate L
    L = R/(2*pi*bandwidth); %Uses the Inductance in Terms of Bandwidth Equation
    
    %Calculate C
    C = (1.00000000)/(((2*pi*freq)^2)*L); %Uses the Resonant LC Frequency Equation
